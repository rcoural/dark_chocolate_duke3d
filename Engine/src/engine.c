...
static void transmaskvline(int32_t x)
{
    ...
    palookupoffs = (intptr_t)FP_OFF(palookup[globalpal]) + (getpalookup((int32_t)mulscale16(swall[x],globvis),globalshade)<<8);
...
static void transmaskvline2 (int32_t x)
{
    ...
    palookupoffse[0] = (intptr_t)FP_OFF(palookup[globalpal]) + (getpalookup((int32_t)mulscale16(swall[x],globvis),globalshade)<<8);
    palookupoffse[1] = (intptr_t)FP_OFF(palookup[globalpal]) + (getpalookup((int32_t)mulscale16(swall[x2],globvis),globalshade)<<8);
...
static void ceilspritehline (int32_t x2, int32_t y)
{
    ...
    asm3 = (intptr_t)FP_OFF(palookup[globalpal]) + (getpalookup((int32_t)mulscale28(klabs(v),globvis),globalshade)<<8);
...
void drawmapview(int32_t dax, int32_t day, int32_t zoome, short ang)
{
    ...
    for(s=sortnum-1; s>=0; s--)
    {
        ...
        if ((spr->cstat&48) == 32)
        {
            ...
            asm3 = (intptr_t) FP_OFF(palookup[spr->pal]+(globalshade<<8));
            globvis = globalhisibility;
            if (sec->visibility != 0) globvis = mulscale4(globvis,(int32_t)((uint8_t )(sec->visibility+16)));
...
{
    intptr_t p;        /* framebuffer pointer: must be pointer-sized on 64-bit */
    int32_t y, dx;
